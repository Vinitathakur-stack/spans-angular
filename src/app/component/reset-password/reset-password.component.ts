...
export class ResetPasswordComponent implements OnInit {
    ...
    save() {
        if (this.resetPasswordForm.valid && this.check_password_validaty()) {
            ...
            input.password = encodeURIComponent(
                this.resetPasswordForm.value["password"]
            );
            input.confirm_password = encodeURIComponent(
                this.resetPasswordForm.value["confirm_password"]
            );
            this.authService.change_password(input).subscribe(
                (result) => {
                    result.status == "error"
                        ? this.toastr.error(result.msg, result.status)
                        : this.toastr.success(result.msg, result.status);
